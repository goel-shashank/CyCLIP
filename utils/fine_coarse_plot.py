...
for k in [1, 2, 3]:
    ...
    for index in range(2):
        ...
        axes[index].bar(X_axis + 0.37, data[index][model], width = 0.35, label = model, alpha = 0.85)
        
        for i, v in enumerate(data[index]["CLIP"]):
            axes[index].text(i + 0.9, data[index][model][i] + 1.5, "+" + str(round(data[index][model][i] - data[index]["CLIP"][i], 2)) + "%", fontsize = 12)
    ...
    plt.savefig(f"analysis/plots/fine_coarse_top{k}.{model}.png")