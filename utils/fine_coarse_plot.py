...
for index in range(2):
    ...
    axes[index].bar(X_axis, data[index]["CLIP"], label = "CLIP", width = 0.3, color = "brown")
    axes[index].bar(X_axis + 0.3, data[index]["CyCLIP"], width = 0.3, label = "CyCLIP")
    
    for i, v in enumerate(data[index]["CLIP"]):
        axes[index].text(i + 0.81, v + 1, v, fontsize = 11)
        axes[index].text(i + 1.15, data[index]["CyCLIP"][i] + 1, str(data[index]["CyCLIP"][i]), fontsize = 11)
