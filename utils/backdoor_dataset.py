...
def run(options):
    options.output_dir_path = os.path.join(options.output_dir_path, options.label)
    
    os.makedirs(options.output_dir_path, exist_ok = True)
    os.makedirs(os.path.join(options.output_dir_path, "images"), exist_ok = True)
    ...
    while(len(bd_images) < options.count):
        try:
            ...
            cv2.imwrite(os.path.join(options.output_dir_path, "images", os.path.basename(images[i])), image)
    ...
    df_captions.to_csv(f"{options.output_dir_path}/backdoor.captions.csv")
    df_templates.to_csv(f"{options.output_dir_path}/backdoor.templates.csv")
