...
def get_loss(umodel, outputs, criterion, options, true_batch_size):   
    if(options.distributed):
        ...
        if(options.inmodal):
            image_embeds, augmented_image_embeds = image_embeds[:len(image_embeds) // 2], image_embeds[len(image_embeds) // 2:]
            text_embeds, augmented_text_embeds = text_embeds[:len(text_embeds) // 2], text_embeds[len(text_embeds) // 2:]
        else:
    ...
    if(options.cylambda):
        ...
        cyclic_loss = options.cylambda * (options.symalpha * inmodal_cyclic_loss + (1 - options.symalpha) * crossmodal_cyclic_loss)
