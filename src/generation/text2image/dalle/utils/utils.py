...
@torch.no_grad()
def clipscore(prompt: str,
               images: np.ndarray,
               clip: torch.nn.Module,
               preprocess,
               device: str) -> np.ndarray:
    images = [preprocess(Image.fromarray((image*255).astype(np.uint8))) for image in images]
    images = torch.stack(images, dim=0).to(device=device)
    texts = clip_.tokenize(prompt).to(device=device)
