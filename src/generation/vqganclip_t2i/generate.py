...
pip install taming-transformers torch_optimizer
'''
import argparse
import math
import random
...
# Add the arguments
vq_parser.add_argument("-file",      "--cap_file", type=str, help="path to captions file", default=None, dest='captions_file')
...
vq_parser.add_argument("-colname",   "--column_name", type=str, help="Initial image", default=None, dest='init_image')
vq_parser.add_argument("-start",     "--start", type=int, help="start index", default=0, dest='start index')
vq_parser.add_argument("-end",       "--end", type=int, help="end index", default=0, dest='end index')
vq_parser.add_argument("-m",         "--clip_model", type=str, help="CLIP model (e.g. ViT-B/32, ViT-B/16)", default='RN50', dest='clip_model')
vq_parser.add_argument("-pre",       "--pretrained", default = False, action = "store_true", help = "Use the OpenAI pretrained models")
...
perceptor, processor = load(args.clip_model, pretrained=True)
...
def get_prompt_obj(prompt):
    txt, weight, stop = split_prompt(prompt)
    txt_out = processor.process_text([txt])
    txt_input_ids, txt_attention_mask = txt_out['input_ids'].to(device), txt_out['attention_mask'].to(device)
    embed = perceptor.get_text_features(txt_input_ids, txt_attention_mask)
    return Prompt(embed, weight, stop).to(device)
...
def checkin(i, losses, prompt, output):
    losses_str = ', '.join(f'{loss.item():g}' for loss in losses)
    tqdm.write(f'i: {i}, loss: {sum(losses).item():g}, losses: {losses_str}')
    out = synth(z)
    info = PngImagePlugin.PngInfo()
    info.add_text('comment', f'{prompt}')
    TF.to_pil_image(out[0].cpu()).save(f'{output[:-4]}_{i}.png', pnginfo=info) 	



def ascend_txt(prompt_obj):
    out = synth(z)
    iii = perceptor.get_image_features(normalize(make_cutouts(out))).float()
    ls = prompt_obj(iii)
    
    return ls # return loss


def train(i, prompt, output):

    opt.zero_grad(set_to_none=True)
    prompt_obj = get_prompt_obj(prompt)
    loss = ascend_txt(prompt_obj)
    
    if (i+1) % args.display_freq == 0:
        checkin(i+1, [loss], prompt, output)
...
def generate_image(caption, fname):
...
if __name__ == '__main__':
    ...
    for ind, cap in enumerate(captions):
        fname = f'{output_dir}/{ind}.png'
        generate_image(cap, fname)
        writer.writerow([cap, fname])
    print('Done')
