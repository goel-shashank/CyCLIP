...
def get_eval_train_dataloader(options, processor):
    ...
    if(options.eval_data_type in ["Imagenet", "ImagenetV2", "ImagenetSketch", "StanfordDogs"]):
        ...
        dataset = torchvision.datasets.MNIST(root = options.eval_test_data_dir, download = True, train = True, transform = processor.process_image)
    elif(options.eval_data_type == "Caltech101"):
        dataset = torchvision.datasets.Caltech101(root = options.eval_test_data_dir, download = True, train = True, transform = processor.process_image)
    elif(options.eval_data_type == "STL10"):
